...
const ROOMS = [
  { id:"R001", name:"DK5 Lecture Hall",      floor:"Level 3", cap:120, score:97, event:"Workshop",  fac:["Projector","Microphone","Air-Con","WiFi"], img:"hall" },
  ...
  { id:"R004", name:"Seminar Room A",        floor:"Level 1", cap:45,  score:78, event:"Seminar",   fac:["Microphone","WiFi","Air-Con"], img:"seminar" },
...
function statusTag(b){
  ...
  // 'cancelled' state covers both a student cancellation and an admin decline —
  // distinguish them using the more specific status field so History shows
  // "Declined" (not "Cancelled") for requests the admin turned down.
  if(b.state === 'cancelled' && b.status === 'declined'){
    return `<span class="tag error">Declined</span>`;
  }
  const [label, cls] = map[b.state] || [b.state,''];
  ...
  return shell("app / dashboard", 'dashboard', `
    <header class="topbar">
      <div class="col gap4">
        ...
        <span class="t13 muted">Wednesday, 24 June 2026 · FSKTM Faculty of Computing</span>
      </div>
      <div class="row gap8 wrap">
    ...
    <div class="scrollpad">
      ...
      <div class="uicard">
        ...
        <div class="card-scroll">${activeList}</div>
      </div>
      <div class="uicard">
        <div class="head"><span class="b7 t14">Recent Activity</span></div>
        <div class="card-scroll">${activity}</div>
      </div>
    </div>`);
}
...
/* ---------- dynamic compatibility score ----------
   Exact-match logic: a room that matches the filter precisely scores 100.
   Any EXCESS beyond what was requested lowers the score — bigger rooms
   and extra facilities are penalised, not rewarded.

   - Capacity excess penalty: how far capacity exceeds the requested minimum,
     as a ratio of the requested capacity, ×15 points.
       e.g. filter 60 pax, room 100 pax -> (100-60)/60 = 0.667 -> -10 pts
   - Facility excess penalty: how many of the room's facilities are NOT
     requested, as a ratio of the TOTAL facility universe (so this always
     applies — even when zero facilities are checked, every facility the
     room has counts as unrequested "excess"), ×20 points.
       e.g. 6 facilities exist total, room has 4 and none were requested
       -> 4/6 = 0.667 -> -13 pts
   - Rooms that fail to MEET the minimum requirements are already excluded by
     the hard filter before this runs, so this function only ever penalises
     for having MORE than what was asked for.
   - If NO filters are set at all (fresh page, default capacity, nothing
     checked), returns null (hide rating in the UI).
------------------------------------------------------------------ */
function computeScore(r, s){
  const capFilterActive = s.cap > 5;
  const facFilterActive = s.facilities.length > 0;
  const anyFilterActive = capFilterActive || facFilterActive;
  ...
  if(capFilterActive){
    const capExcessRatio = Math.max(0, (r.cap - s.cap) / s.cap);
    penalty += capExcessRatio * 15;
  }

  // Facility excess always applies once ANY filter is active (capacity or
  // facility), so a room's unrequested equipment is never a free pass.
  const extraFac = r.fac.filter(f => !s.facilities.includes(f));
  const facUniverseSize = FACILITIES.length; // total distinct facilities that exist
  const facExcessRatio = extraFac.length / facUniverseSize;
  penalty += facExcessRatio * 20;
...
function renderP2(){
  ...
  const filters = `
    <aside class="filterpanel">
      ...
      <div class="col gap10">
        <div class="row between">
          <span class="b6 t13">Min. Capacity</span>
          ${s.capEditing
            ? `<input type="number" class="field" data-input="p2capType" min="5" max="150" value="${s.cap}" style="width:72px;padding:3px 8px;font-size:12px;text-align:center;" />`
            : `<span class="tag dark p2-cap-badge" data-act="p2capEdit" style="cursor:pointer;">${s.cap} pax ✎</span>`}
        </div>
  ...
  const results = `
    <main class="resultscol">
      <div class="col gap4"><span class="b7 t15 disp">${filtered.length} room${filtered.length!==1?'s':''} matched</span><span class="t11 muted mono">${filtered.length && filtered[0].score!=null ? 'Ranked by compatibility score' : 'Set a capacity or facility filter to rank by score'}</span></div>
      ${filtered.length ? filtered.map((r,i)=>`<div class="uicard" data-act="selectRoom" data-room="${r.name}" style="cursor:pointer;${i===0 && r.score!=null?'border-color:var(--primary);border-width:2px;':''}">
        <div class="row" style="align-items:flex-start;min-width:0;">
          ...
          <div class="row" style="flex:1;padding:14px;gap:12px;align-items:flex-start;min-width:0;">
            <div class="col gap6" style="flex:1;min-width:0;">
              <div class="row gap8 wrap">${i===0 && r.score!=null?`<span class="tag" style="background:var(--grad-secondary);border-color:transparent;color:#fff;font-size:10px;letter-spacing:.06em;">${svgIco('star','sm')} BEST MATCH</span>`:''}</div>
            ...
            <div class="col gap8" style="align-items:flex-end;flex-shrink:0;">
              ${r.score!=null ? `
              <div class="row gap4" style="align-items:baseline;"><span class="b8 t20 disp" style="color:${r.score>=80?'var(--success-deep)':r.score>=60?'var(--warning-deep)':'var(--error-deep)'};">${r.score}</span><span class="t10 muted mono">/100</span></div>
              <div style="width:64px;height:7px;background:var(--fill-2);border-radius:999px;overflow:hidden;"><div style="height:100%;width:${r.score}%;background:${r.score>=80?'var(--grad-success)':r.score>=60?'var(--grad-warning)':'var(--grad-error)'};border-radius:999px;transition:width .4s ease;"></div></div>
              ` : ''}
...
function datePickerHTML(){
  ...
  const rangeStartKey = '2026-06-23'; // bookings open from this date onwards
  const rangeEndKey   = '2026-06-30'; // bookable window closes after this date
  ...
  for(let day=1; day<=daysInMonth; day++){
    ...
    const outOfRange = key < rangeStartKey || key > rangeEndKey;
    const disabled = isWeekend || outOfRange;
  ...
  const atMin = true; // bookable range is entirely within June 2026 — month nav locked
  const atMax = true;
  return `<div class="datepicker">
    <div class="row between" style="margin-bottom:10px;">
      <button class="btn sm" style="padding:5px 8px;" data-act="p3dateNav" data-v="-1" ${atMin?'disabled':''}>‹</button>
      <span class="b7 t14 disp">${monthLabel}</span>
      <button class="btn sm" style="padding:5px 8px;" data-act="p3dateNav" data-v="1" ${atMax?'disabled':''}>›</button>
    </div>
          ...
          <div class="uicard">
            ...
            <div class="row gap14" style="padding:16px 18px;flex-wrap:wrap;">
              ...
              <div class="col gap6" style="flex:1;min-width:0;"><span class="b7 t16 disp">${room.name}</span>
                <div class="row gap8 wrap"><span class="tag">${svgIco('pin','sm')}${room.floor}</span><span class="tag">${svgIco('users','sm')}${room.cap} pax max</span><span class="tag dark">${room.event}</span></div>
              </div>
            </div>
          </div>
          <div class="uicard${s.pickerOpen?' uicard-overflow-visible':''}">
            ...
            <div style="padding:16px 18px;" class="col gap14">
              ...
              <div class="row between p3-time-bar" style="padding:12px 16px;border-radius:10px;background:var(--primary-tint);flex-wrap:wrap;gap:8px;">
                <span class="mono b6 t14 p3-time-start" style="color:var(--primary-deep);">${fmtT(s.start)}</span>
                <div style="flex:1;min-width:20px;height:2px;background:var(--primary);opacity:.3;margin:0 12px;border-radius:2px;"></div>
                <span class="mono b6 t14 p3-time-end" style="color:var(--primary-deep);">${fmtT(s.end)}</span>
              </div>
            </div>
          </div>
          <div class="uicard">
...
function renderP4(){
  ...
  weekDays.forEach(day=>{
    ...
    HOURS.forEach(hour=>{
      ...
      cols.forEach(room=>{
        ...
        let inner = (bk && isStart) ? `<div class="ev"><span class="ev-title">${bk.event}${bk.occu==='vacant'?' ⚠':''}</span><span class="ev-booker">${bk.bookerName||CURRENT_USER.name}</span></div>` : '';
  ...
  if(sel){
    ...
    if(b){
      panel = `<div class="slotpanel">
        ...
        <div class="row gap8" style="padding:8px 10px;border-radius:8px;background:var(--panel);margin-bottom:8px;">
          <div class="ph avatar" style="width:26px;height:26px;flex-shrink:0;background:var(--secondary);color:#fff;border:0;font-size:10px;">${(b.bookerName||CURRENT_USER.name).split(' ').map(n=>n[0]).join('').slice(0,2)}</div>
          <div class="col gap2" style="min-width:0;"><span class="b6 t12">${b.bookerName||CURRENT_USER.name}</span><span class="t10 muted mono">${b.bookerRole||CURRENT_USER.role}</span></div>
        </div>
        ${occuPulse(b)}
  ...
  return shell("app / calendar", 'calendar', `
    ...
    ${hint}
    <div class="cal-scroll" style="background:#fff;flex:1;position:relative;"><div class="cal" style="--col-count:${cols.length};">${grid}</div>${panel}</div>`);
}
...
function renderMyBookings(){
  ...
  const bookingRow = (b, isActive)=>{
    ...
    <div class="lrow">
      <div class="stripe ${stripeCls}"></div>
      <div class="col gap6" style="flex:1;min-width:0;">
        <div class="row gap8 wrap"><span class="b7 t14">${b.roomName}</span><span class="tag solid">${b.event}</span>${statusTag(b)}${b.reported?'<span class="tag">report filed</span>':''}${b.status==='auto-released'?'<span class="tag error">auto-released</span>':''}</div>
  ...
  const pendingRow = (b)=>`
    <div class="lrow">
      ...
      <div class="col gap6" style="flex:1;min-width:0;">
        <div class="row gap8 wrap"><span class="b7 t14">${b.roomName}</span><span class="tag solid">${b.event}</span>${statusTag(b)}</div>
        <div class="row gap12 wrap muted mono t12">
          <span>▦ ${dateLabel(b.day)}</span><span>◷ ${fmtT(b.start)} – ${fmtT(b.end)}</span><span>⌂ ${b.floor}</span>
        </div>
        ${Object.keys(b.addons||{}).length?`<div class="row gap6 wrap mt4">${addonChips(b.addons,'sm')}</div>`:''}
  ...
  const inner = empty
    ? `<div class="scrollpad">${emptyBox('◷','No bookings yet','You have not reserved any venues yet. Start a booking and it will appear here.')}</div>`
    : `<div class="scrollpad">
        <div class="uicard">
          <div class="head"><span class="b7 t15">Pending</span><span class="tag warning">${pending.length}</span></div>
          <div class="card-scroll">${pending.length?pending.map(b=>pendingRow(b)).join(''):('<div class="lrow muted t12" style="justify-content:center;padding:22px;">No pending bookings.</div>')}</div>
        </div>
        <div class="uicard">
          <div class="head"><span class="b7 t15">Active</span><span class="tag success">${active.length}</span></div>
          <div class="card-scroll">${active.length?active.map(b=>bookingRow(b,true)).join(''):('<div class="lrow muted t12" style="justify-content:center;padding:22px;">No active bookings.</div>')}</div>
        </div>
        <div class="uicard">
          <div class="head"><span class="b7 t15">History</span><span class="tag">${history.length}</span></div>
          <div class="card-scroll">${history.length?history.map(b=>bookingRow(b,false)).join(''):('<div class="lrow muted t12" style="justify-content:center;padding:22px;">No past bookings.</div>')}</div>
        </div>
      </div>`;
...
function renderReports(){
  ...
  const inner = reps.length===0
    ? `<div class="scrollpad">${emptyBox('⚠','No reports submitted','After a booking ends you can report room condition issues — your reports appear here.')}</div>`
    : `<div class="scrollpad">
        ${reps.map(r=>`<div class="uicard">
          ...
          <div style="padding:14px 18px;" class="col gap10">
            ...
            ${r.details?`<div class="col gap4"><span class="t11 b6 muted">Description:</span><span class="t13">${r.details}</span></div>`:''}
          </div>
        </div>`).join('')}
      </div>`;
...
/* ============================================================
   P5 — ADMIN BOARD (tabbed: Booking Requests + Room Condition Reports)
   ============================================================ */
function renderP5(){
  const s = WF.p5;
  const tab = s.adminTab || 'bookings';
  ...
  const allReports = WF.session.reports;

  const statRows = [["Pending",pending.length,"warning"],["Approved",approved.length,"success"],["Declined",declined.length,"error"]];
  ...
  const cardArea = cur ? `
    ...
    <div class="swipe">
      ...
      <div class="row gap10" style="padding:10px 12px;border-radius:10px;background:var(--panel);border:1px solid var(--line);margin-bottom:14px;">
        <div class="ph avatar" style="width:38px;height:38px;flex-shrink:0;background:var(--secondary);color:#fff;border:0;font-size:13px;">${(cur.bookerName||CURRENT_USER.name).split(' ').map(n=>n[0]).join('').slice(0,2)}</div>
        <div class="col gap2" style="min-width:0;flex:1;">
          <span class="b7 t14">${cur.bookerName||CURRENT_USER.name}</span>
          <div class="row gap8 wrap"><span class="tag solid">${cur.bookerRole||CURRENT_USER.role}</span><span class="t11 muted mono">${cur.bookerId||CURRENT_USER.id}</span></div>
        </div>
      </div>
  ...
  : `<div class="col gap12" style="align-items:center;text-align:center;padding:40px;">
      <div class="ph" style="width:84px;height:84px;border-radius:50%;background:var(--success);color:#fff;border:0;padding:0;display:flex;align-items:center;justify-content:center;"><span class="svgico" style="width:44px !important;height:44px !important;color:#fff;">${SVG_ICONS.check}</span></div>
      <span class="b8 t20 disp">All caught up!</span><span class="t13 muted">No pending booking requests at this time.</span>
    </div>`;

  /* ---- TAB: Room Condition Reports ---- */
  const reportsArea = allReports.length === 0
    ? `<div class="col gap12" style="align-items:center;text-align:center;padding:40px;">
        <div class="ph" style="width:64px;height:64px;border-radius:50%;background:var(--fill);border:0;padding:0;display:flex;align-items:center;justify-content:center;">${svgIco('flag','sm')}</div>
        <span class="b7 t16 disp">No reports yet</span>
        <span class="t13 muted">Room condition reports from users will appear here.</span>
      </div>`
    : `<div class="col gap12" style="padding:20px;overflow-y:auto;flex:1;">
        ${allReports.map(r=>{
          const booking = allBookings.find(b=>b.roomName===r.roomName);
          return `<div class="uicard admin-report-card" style="padding:0;">
            <div class="head" style="background:var(--secondary-tint);">
              <div class="row gap8 wrap">
                <span class="b7 t14">${r.roomName}</span>
                <span class="tag dark">${r.status}</span>
                ${r.issues.map(id=>{const c=ISSUE_CHIPS.find(x=>x[0]===id);return `<span class="tag error">${c?c[1]:id}</span>`;}).join('')}
              </div>
              <span class="t11 muted mono">${r.id} · ${r.time}</span>
            </div>
            <div style="padding:14px 18px;" class="col gap10">
              <div class="row gap8 wrap">
                <div class="ph avatar" style="width:32px;height:32px;flex-shrink:0;background:var(--secondary);color:#fff;border:0;font-size:11px;">${(r.bookerName||CURRENT_USER.name).split(' ').map(n=>n[0]).join('').slice(0,2)}</div>
                <div class="col gap2">
                  <span class="b6 t13">${r.bookerName||CURRENT_USER.name}</span>
                  <div class="row gap8"><span class="tag solid">${r.bookerRole||CURRENT_USER.role}</span><span class="t11 muted mono">${r.bookerId||CURRENT_USER.id}</span></div>
                </div>
              </div>
              <div class="divline"></div>
              <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;">
                <div class="metsq">${svgIco('calendar','sm')}${r.day?dateLabel(r.day):'–'}</div>
                <div class="metsq">${svgIco('clock','sm')}${r.start&&r.end?`${fmtT(r.start)} – ${fmtT(r.end)}`:'–'}</div>
                <div class="metsq">${svgIco('pin','sm')}${r.floor||'–'}</div>
              </div>
              ${r.details?`<div class="col gap4"><span class="t11 b6 muted">Description</span><span class="t13">${r.details}</span></div>`:''}
            </div>
          </div>`;
  ...
  return frame("app / admin", `
    <div class="admin-topbar">
      <div class="row gap12 wrap">
        <div class="row gap8"><span class="svgico" style="width:20px;height:20px;color:#9fb3c8;">${SVG_ICONS.shield}</span>
          <div class="col gap4"><span class="b8 t16 disp" style="color:#fff;">Admin Board</span><span class="t11 mono" style="color:#9fb3c8;">FSKTM Faculty Venue Management</span></div>
        </div>
      </div>
    ...
    <div class="admin-tabs-bar">
      <button class="admin-tab-btn ${tab==='bookings'?'on':''}" data-act="p5tab" data-v="bookings">${svgIco('bookings','sm')}Booking Requests<span class="admin-tab-badge">${pending.length}</span></button>
      <button class="admin-tab-btn ${tab==='reports'?'on':''}" data-act="p5tab" data-v="reports">${svgIco('flag','sm')}Room Condition Reports<span class="admin-tab-badge">${allReports.length}</span></button>
    </div>
    <div class="admin-appbody">
      <aside class="admin-sidebar">
        <div class="admin-sidebar-inner">
          ${statRows.map(([l,v,tone])=>`<div class="admin-stat-row" style="border-left: 4px solid ${tone==='success'?'var(--success)':tone==='warning'?'var(--warning)':tone==='error'?'var(--error)':'var(--line-2)'};">
            <span class="t13">${l}</span>
            <span class="b8 t18 disp" style="${tone==='success'?'color:var(--success-deep);':tone==='warning'?'color:var(--warning-deep);':tone==='error'?'color:var(--error-deep);':''}">${v}</span>
          </div>`).join('')}
        </div>
      </aside>
    ...
    <div class="modal">
      ...
      <div style="padding:18px;" class="col gap16">
        ...
        <div class="col gap6"><span class="t12 b6 muted">Additional details (optional)</span>
          <textarea class="field area" data-input="p6details" placeholder="Describe the issue in more detail…" style="height:64px;padding-top:11px;resize:vertical;">${s.details||''}</textarea>
        </div>
