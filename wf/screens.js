...
/* ---------- dynamic compatibility score ----------
   Pure closeness-to-filter logic, as specified:
   - Capacity: room qualifies (capacity >= requested) -> full marks on this axis.
     (Hard filter already excludes rooms below the requested capacity, so any
     room reaching this point already meets it — extra capacity is not penalised.)
   - Facilities: % of the requested facilities the room has. Extra facilities
     the room has beyond what was asked do not reduce or inflate the score.
   - Final score = average of the two axes that were actually requested.
...
function computeScore(r, s){
  ...
  if(capFilterActive){
    // Room already passed the hard "cap >= requested" filter to be in this list.
    total += 100;
    axes++;
  }
  ...
  if(facFilterActive){
    ...
    total += Math.round((matched / s.facilities.length) * 100);
    axes++;
  }

  return Math.round(total / axes);
}
