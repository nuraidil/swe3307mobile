...
const FLOORS      = ["Any Floor","Level 1","Level 2","Level 3"];
...
/* ---------- dynamic compatibility score ----------
   ...
   - Facility excess penalty: how many EXTRA facilities the room has beyond
     what was requested, as a ratio of the requested facility count, ×20 points.
       e.g. filter 3 facilities, room has 3 extra unrequested ones -> ratio 1.0 -> -20 pts
   ...
   - If NO filters are set at all, returns null (hide rating in the UI).
------------------------------------------------------------------ */
function computeScore(r, s){
  const capFilterActive = s.cap > 5;
  const facFilterActive = s.facilities.length > 0;

  if(!capFilterActive && !facFilterActive) return null;
  ...
  if(facFilterActive){
    ...
    const facExcessRatio = extraFac.length / s.facilities.length;
      ...
      <div class="p3grid">
        <div class="col gap16">
          <div class="uicard">
            ...
            <div style="padding:0 18px 16px;" class="col gap8">
              <div class="row between">
                <span class="t12 b6 muted">Expected number of attendees</span>
                ${s.paxEditing
                  ? `<input type="number" class="field" data-input="p3paxType" min="1" max="${room.cap}" value="${s.pax}" style="width:72px;padding:3px 8px;font-size:12px;text-align:center;" />`
                  : `<span class="tag dark" data-act="p3paxEdit" style="cursor:pointer;">${s.pax} pax ✎</span>`}
              </div>
              <input type="range" min="1" max="${room.cap}" value="${s.pax}" data-change="p3pax"
                style="width:100%;accent-color:var(--primary);cursor:pointer;margin:4px 0;" />
              <div class="row between mono t10 muted"><span>1</span><span>${room.cap} max</span></div>
              ${s.paxError ? `<span class="t11" style="color:var(--error-deep);">${s.paxError}</span>` : ''}
            </div>
          </div>
          <div class="uicard">
