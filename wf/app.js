...
const WF = {
  ...
  p2: { cap: 5, events: [], facilities: [], floor: 'Any Floor', capEditing: false },
  p3: { addons: {}, day: '2026-06-23', start: '09:00', end: '11:00', email: true, conflictError: null, pickerOpen: false, pickerMonth: 5, pickerYear: 2026, pax: '1', paxEditing: false },
...
document.addEventListener('click', (e)=>{
  ...
  switch(act){
    ...
    // P2 filters
    case 'selectRoom': WF.selectedRoom = t.dataset.room; WF.p3.conflictError = null; WF.p3.pax = '1'; WF.p3.paxEditing = false; goto('summary'); break;
    ...
    case 'p3paxEdit': WF.p3.paxEditing = true; render(); setTimeout(()=>{ document.querySelector('[data-input="p3paxType"]')?.focus(); }, 0); break;
    ...
    case 'confirm': {
      ...
      let { day, start, end, pax } = WF.p3;
      if(end <= start){ const i=TIME_OPTS.indexOf(start); end = TIME_OPTS[Math.min(i+2,TIME_OPTS.length-1)]; }
      // Pax validation
      if(pax === '' || pax == null){
        WF.p3.paxError = 'Please enter the expected number of attendees.';
        render();
        break;
      }
      const paxNum = Number(pax);
      if(!Number.isFinite(paxNum) || paxNum < 1 || paxNum > room.cap){
        WF.p3.paxError = paxNum > room.cap ? `Exceeds room capacity (${room.cap} pax max).` : 'Enter a valid number of attendees.';
        render();
        break;
      }
      ...
      createBooking({ roomName:room.name, floor:room.floor, cap:room.cap, pax:paxNum, event:room.event, day, start, end, addons:{...WF.p3.addons} });
      WF.p3.addons = {};
      WF.p3.pax = '';
      WF.p3.paxError = null;
...
document.addEventListener('input', (e)=>{
  ...
  const pl = e.target.closest('[data-change="p3pax"]');
  if(pl){
    WF.p3.pax = String(Number(pl.value));
    WF.p3.paxError = null;
    WF.p3.paxEditing = false;
    render();
    return;
  }
...
/* typed-number badge commit (P2 capacity, P3 pax) — Enter or blur applies the value */
function commitTypedCap(el, kind){
  const room = kind==='p3' ? ((window.ROOMS||[]).find(r=>r.name===WF.selectedRoom) || window.ROOMS[0]) : null;
  const max = kind==='p3' ? room.cap : 150;
  const min = kind==='p3' ? 1 : 5;
  let n = Number(el.value);
  if(!Number.isFinite(n)) n = min;
  n = Math.max(min, Math.min(max, Math.round(n)));
  if(kind==='p2'){ WF.p2.cap = n; WF.p2.capEditing = false; }
  else { WF.p3.pax = String(n); WF.p3.paxError = null; WF.p3.paxEditing = false; }
  render();
}
document.addEventListener('keydown', (e)=>{
  if(e.key==='Enter'){
    const t1 = e.target.closest('[data-input="p2capType"]');
    if(t1){ commitTypedCap(t1, 'p2'); return; }
    const t2 = e.target.closest('[data-input="p3paxType"]');
    if(t2){ commitTypedCap(t2, 'p3'); return; }
  }
});
