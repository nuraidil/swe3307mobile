...
const WF = {
  ...
  p5: { idx: 0, approved: [], declined: [], adminTab: 'bookings' },
  p6: { chips: [] },
...
function submitReport(issues, details){
  ...
  WF.session.reports.unshift({
    id, roomName:room, issues:[...issues], details, time:'Just now', status:'Submitted',
    bookerName: b ? b.bookerName : CURRENT_USER.name,
    bookerId:   b ? b.bookerId  : CURRENT_USER.id,
    bookerRole: b ? b.bookerRole : CURRENT_USER.role,
    day:   b ? b.day   : null,
...
const SCROLL_PRESERVE_SELECTOR = '.scrollpad, .filterpanel, .resultscol, .subbody-scroll, .admin-sidebar, .admin-main, .authwrap, .card-scroll';
...
/* ---------- render ---------- */
/* ---------- render ----------
   Uses requestAnimationFrame to batch multiple rapid state changes
   (e.g. slider dragging, typing) into one repaint per frame, preventing
   the visible white-flash / flicker that occurs when innerHTML is replaced
   many times per second.  A pending flag ensures we never queue more than
   one frame per burst of updates. */
let _renderPending = false;
let _lastRenderedHtml = '';
let _lastScreen = '';      // tracks last rendered screen for nav detection
let _lastDamage = false;   // tracks last modal state to detect first open
let _isNavigation = false; // true only when goto() was called — suppresses enter animation for in-page renders
...
function _doRender(){
  ...
  _isNavigation = false; // consume immediately before any early return
  ...
  // Add modal HTML before comparing so damage state is part of the diff
  let modalHtml = '';
  if(WF.damage){
    modalHtml = WF.damage==='submitted' ? S.renderP6Submitted() : S.renderP6Overlay();
  }
  const fullHtml = html + modalHtml;

  // Skip the DOM write entirely if the output is identical to last render —
  // avoids repaints when state change didn't actually affect the current view.
  if(fullHtml === _lastRenderedHtml){
    return;
  }
  _lastRenderedHtml = fullHtml;

  stage.innerHTML = html;
  restoreScrollPositions(scrollMap);

  // Detect whether this render is a page navigation or an in-page interaction.
  // Animations should only play on navigation (or modal first-open), not on
  // facility toggles, date picks, chip clicks, etc.
  if(!isNav){
  ...
  if(WF.damage){
    ...
    if(screenEl){
      ...
      // Suppress modal entrance animation if the modal was already open
      // (i.e. this is a chip click or textarea input, not the modal opening)
      const modalJustOpened = !_lastDamage;
      if(modalEl && !modalJustOpened){
        modalEl.classList.add('no-anim-self');
      }
      screenEl.appendChild(modalEl);
    }
  }
  _lastDamage = !!WF.damage;
  updateChrome();
}
...
function goto(screen){
  _isNavigation = true; // mark this render as a page transition (enables enter animation)
  WF.screen = screen;
...
document.addEventListener('click', (e)=>{
  ...
  switch(act){
    ...
    case 'logout': _isNavigation = true; WF.screen='login'; WF.authTab='login'; render(); break;
    ...
    // P5 admin
    case 'p5tab': WF.p5.adminTab = v; render(); break;
...
document.addEventListener('change', (e)=>{
  ...
  if(sel){
    ...
    if(key==='p3start'){
      WF.p3.start = sel.value; WF.p3.conflictError = null;
      if(WF.p3.end<=WF.p3.start){ const i=TIME_OPTS.indexOf(WF.p3.start); WF.p3.end=TIME_OPTS[Math.min(i+1,TIME_OPTS.length-1)]; }
      // Update summary bar directly without full re-render to avoid flicker
      _patchTimeBar();
      render();
      // After render, ensure selects show the right value (browser may show stale option)
      requestAnimationFrame(()=>{
        const s = document.querySelector('[data-change="p3start"]');
        const e2 = document.querySelector('[data-change="p3end"]');
        if(s) s.value = WF.p3.start;
        if(e2) e2.value = WF.p3.end;
      });
    }
    if(key==='p3end'){
      WF.p3.end = sel.value; WF.p3.conflictError = null;
      _patchTimeBar();
      render();
      requestAnimationFrame(()=>{ const e2=document.querySelector('[data-change="p3end"]'); if(e2) e2.value=WF.p3.end; });
    }
    return;
  }
...
/* Patch the time bar display without a full re-render (avoids flicker on selects) */
function _patchTimeBar(){
  const bar = document.querySelector('.p3-time-bar');
  if(!bar) return;
  bar.querySelector('.p3-time-start').textContent = fmtT(WF.p3.start);
  bar.querySelector('.p3-time-end').textContent = fmtT(WF.p3.end);
}

/* live slider drag — update only the badge label in-place, render() on release */
document.addEventListener('input', (e)=>{
  const sl = e.target.closest('[data-change="p2cap"]');
  if(sl){
    WF.p2.cap = Number(sl.value);
    WF.p2.capEditing = false;
    // Update only the pax badge label directly — no full re-render mid-drag
    const badge = document.querySelector('.p2-cap-badge');
    if(badge) badge.textContent = WF.p2.cap + ' pax';
    return;
  }
...
/* Full render on slider release (mouseup/touchend) so scores update */
document.addEventListener('change', (e)=>{
  const sl = e.target.closest('[data-change="p2cap"]');
  if(sl){ WF.p2.cap = Number(sl.value); WF.p2.capEditing = false; render(); }
}, true); // capture phase so this runs before the inner change handler
...
/* Bottom-fade mask: remove it when the user has scrolled to the bottom
   so the last item isn't permanently semi-transparent. */
document.addEventListener('scroll', (e)=>{
  const el = e.target;
  if(el && el.classList && el.classList.contains('card-scroll')){
    const atBottom = el.scrollHeight - el.scrollTop <= el.clientHeight + 4;
    el.classList.toggle('at-bottom', atBottom);
  }
}, true); // capture phase so we catch scroll on any card-scroll element
