...
const WF = {
  ...
  p3: { addons: {}, day: '2026-06-24', start: '09:00', end: '11:00', email: true, conflictError: null, pickerOpen: false, pickerMonth: 5, pickerYear: 2026, pax: '' },
...
const BOOKING_START_KEY = '2026-06-24';
const BOOKING_END_KEY   = '2026-07-31';
...
document.addEventListener('click', (e)=>{
  ...
  switch(act){
    ...
    case 'p3dateNav': {
      ...
      // clamp to months covered by the bookable range (June-July 2026)
      const isInRange = (y===2026 && (m===5 || m===6));
...
document.addEventListener('change', (e)=>{
  ...
  if(inp){
    ...
    if(key==='p3pax'){
      ...
      if(v===''){ WF.p3.pax = ''; WF.p3.paxError = null; }
      else {
        const n = Number(v);
        if(!Number.isFinite(n) || n < 1){ WF.p3.paxError = 'Enter a valid number of attendees.'; }
        else if(n > room.cap){ WF.p3.paxError = `Exceeds room capacity (${room.cap} pax max).`; }
        else { WF.p3.paxError = null; }
        WF.p3.pax = v;
      }
      render();
    }
